# -*- coding: utf-8 -*-
"""
图9：SST-2 样本多方法归因结果对比（多子图水平柱状图）

取索引为 0 的样本，分别从 attention_sst2.json（第11层注意力）、ig_sst2.json、
shap_sst2.json、lime_sst2.json 中提取归因分数，以 4x1 的水平柱状图子图布局
展示四种方法的归因结果对比。

各方法的 tokens 列表可能不完全相同（注意力含 [CLS] 和 [SEP]，LIME 不含），
以各方法自身的 tokens 列表为纵轴。每个子图最多显示前25个 token。

数据来源：
    - attention_sst2.json（索引0，第11层）
    - ig_sst2.json（索引0）
    - shap_sst2.json（索引0）
    - lime_sst2.json（索引0）

输出文件名：图3-SST2样本多方法归因结果对比.svg / .png
...
MAX_TOKENS = 25  # 每个子图最多显示的 token 数量
...
def load_all_attribution_data():
    ...
    with open(os.path.join(DATA_DIR, "ig_sst2.json"), "r", encoding="utf-8") as f:
        ig_data = json.load(f)
    with open(os.path.join(DATA_DIR, "shap_sst2.json"), "r", encoding="utf-8") as f:
        shap_data = json.load(f)
    with open(os.path.join(DATA_DIR, "lime_sst2.json"), "r", encoding="utf-8") as f:
...
def main():
    ...
    fig.suptitle('SST-2 样本多方法归因结果对比（样本索引：0）', fontsize=12, y=1.01)
    plt.tight_layout(pad=1.5)

    fig.savefig(os.path.join(FIGURES_DIR_SVG, "图3-SST2样本多方法归因结果对比.svg"),
                format='svg', bbox_inches='tight')
    fig.savefig(os.path.join(FIGURES_DIR_PNG, "图3-SST2样本多方法归因结果对比.png"),
