# -*- coding: utf-8 -*-
"""
图2：SST-2 数据集忠实度 AUC 下降曲线（折线图）

从 faithfulness_results.json 中读取 SST-2 数据集上四种可解释性方法的
遮蔽比例-预测概率下降幅度曲线，展示各方法的忠实度表现。

数据来源：
    - faithfulness_results.json（auc_drop_curves 中 *_sst2 字段，masking_ratios 字段）

输出文件名：图4-SST2数据集忠实度AUC下降曲线.svg / .png
...
def main():
    """
    绘制 SST-2 数据集忠实度 AUC 下降曲线，并保存为 SVG 和 PNG 格式。
    """
    ...
    # 四种方法配置：名称、数据键、线型、标记
    methods = [
        ('注意力权重', 'attention_sst2', '-', 'o'),
        ('集成梯度', 'integrated_gradients_sst2', '--', '^'),
        ('SHAP', 'shap_sst2', '-.', 's'),
        ('LIME', 'lime_sst2', (0, (5, 3)), 'D'),  # 长虚线
    ]
    ...
    ax.set_title('SST-2 数据集忠实度 AUC 下降曲线', fontsize=12)
    ...
    fig.savefig(os.path.join(FIGURES_DIR_SVG, "图4-SST2数据集忠实度AUC下降曲线.svg"),
                format='svg', bbox_inches='tight')
    fig.savefig(os.path.join(FIGURES_DIR_PNG, "图4-SST2数据集忠实度AUC下降曲线.png"),
